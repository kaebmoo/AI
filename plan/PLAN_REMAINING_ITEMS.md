...
| 9.6 | ✅ 2026-09-19 (Phase 4d `5428bb3`) — schema browser / dimension families อ่านผ่าน source ของตาราง, onboarding ปฏิเสธตารางของ file source, DDL ของ brain มาจาก registry; เดิม: admin schema browser, dimension families, onboarding, sync-brain เห็นแค่ business DB เดิม | `app/api/v1/admin/*` | สำหรับ `feed_*` เห็นสำเนาเก่า ไม่ใช่ไฟล์ — ต้องผูกกับ source ของ context (ทำพร้อม Plan 7 Phase 4 admin) | Medium |
...
| 9.9 | ✅ 2026-09-19 — เจ้าของสั่งจัดการ: ลบ `matcha_examples.py` + `scripts/test_matcha_improvements.py`, `business_db.py` + test 6 ข้อของมัน, adapter SQLite/PostgreSQL/MSSQL + `create_adapter`/`get_adapter_for_settings` (−1,308 บรรทัด; ไม่มี caller ใน app/mcp/scripts/tests) — SQL source ในอนาคตสร้าง adapter ใหม่ตามสัญญาเดียวกับ `DuckDBFileAdapter` (read-only, ผ่าน gate, pin ต่อ build); ของเดิมอยู่ใน git history | `app/services/` | pytest 819 → 813 (−6 = test ของ `business_db`) | Low |
...
- ✅ **ebt ปิดแล้ว 2026-09-19 → REMAIN-11 ปิดทั้งข้อ** — NT-Report ebt 1.3.1 (`73ddc96`, build `20260919T013457Z`): สะสม 3 คอลัมน์เป็น `agg: point_in_time`, กฎห้าม SUM/BETWEEN ข้ามงวด, รายเดือนใช้ `*_month`, สูตรรายสายงาน/ศูนย์ต้นทุน;
  ฝั่ง AI re-sync เอง (ไม่ได้ลงทะเบียนใหม่) → `feed_ebt` active, golden 24 ข้อ, eval **22/24 = 91.7%** (`eval_20260919_0912`, P50 6.4 s, ไม่มี retry) — 18/24 บน 1.3.0
  - ที่ยังพลาดซ้ำ ๆ (2–3 รอบ): "ค่าใช้จ่าย … ของเดือน" → โมเดลเลือกคอลัมน์ `expense` (สะสม) แทน `expense_month`; และแปลง พ.ศ. 2568 → 2026 เป็นครั้งคราว
  - คำถามรายสายงาน/ศูนย์ต้นทุนไม่มี golden (control totals มีแต่ยอดรวม): ถามซ้ำ 8 ครั้งใช้ filter ครบ 8/8, สายงาน 1 สะสม = −214,373,476.47 ตรงข้อมูลจริง 4/4 (รวมสายงาน 2 = 417.33 MB ตรงยอดทางการ) แต่ "รายได้สายงาน 2 เดือน ก.ค." ได้ยอดก่อนหัก `08.รายได้อื่น` 1 ใน 2 รอบ (616.57 vs 612.07 M) → ถ้าต้องการวัดได้ ให้ NT-Report ประกาศ control totals ราย division
...
✅ `ff34aab`: (1) Pass 1 ได้รายชื่อคอลัมน์ที่มีเฉพาะในตารางอื่นของ context (เดิมบางรอบไม่ดึง filter สายงานเลย → ไม่มีอะไรให้ guard ตรวจ); (2) guard นับเฉพาะ filter ที่**ค่าอยู่ในคำถามจริง** — Pass 1 เคยแปลงกฎ "รายงานครอบคลุม 2 สายงาน" เป็น filter เอง ทำให้ SQL ยอดรวมที่ถูกต้องถูกปฏิเสธจน retry หมด (1/24)
⬜ ที่เหลือ: ชื่อคอลัมน์ที่ไม่มีตารางไหนมี ยังปล่อยให้ Pass 2 map เอง (เช่น `business_unit` → `bu`); value lookup / keyword index ยังทำเฉพาะ main view (ค่าของ `fact_ebt.division` ไม่อยู่ใน index); dimension ที่ Pass 1 ไม่ดึงยังหลุดได้
