...
| feed_ebt | 1.4.0 | 33/36 = 91.7% (`1002`) | 32/36 = 88.9% (`1015`, harness เดิม) |

ไม่มี retry / guard ยิง 0 ครั้งทุกชุด.

**หลังแก้ harness (`176ec3d`, เจ้าของสั่ง 2026-09-19):** คอลัมน์ label ที่เป็นข้อความล้วนและไม่ใช่ชื่อคอลัมน์ที่ golden คาดหวัง ถูกวางไว้ข้าง ๆ ได้ (นับเป็น value_match ไม่ใช่ exact_match; ตัวเลขเกิน / คอลัมน์ขาด / จำนวนแถวต่าง / ค่าผิด = mismatch เหมือนเดิม) — eval ซ้ำบน config จริง:

| Context | schema | ผล | มีคอลัมน์ label เพิ่ม | P50 / P95 | ไฟล์ |
|---|---|---|---|---|---|
| feed_ebt | 1.4.0 | **35/36 = 97.2%** | 3 | 6.7 / 7.8 s | `eval_20260919_1026` |
| feed_revenue | 2.3.0 | **14/14** | 1 | 5.3 / 6.5 s | `eval_20260919_1028` |
| feed_sales | 1.3.0 | **12/12** | 0 | 6.1 / 6.5 s | `eval_20260919_1029` |
| feed_expense | 1.2.0 | **12/12** | 1 | 5.3 / 6.3 s | `eval_20260919_1030` |

ไม่มี retry, guard ยิง 0 ครั้ง. ข้อเดียวที่ยังพลาด (ebt): "ของเดือนกันยายน 2568" → โมเดลแปลงเป็น `202609` (พ.ศ. 2568 = ค.ศ. 2025) — ความผิดของโมเดล ไม่ใช่ harness. ก่อนแก้ harness รอบเดียวกันบน config จริง: ebt 32/36 (88.9%), revenue 13/14 — ข้อที่ต่างทั้งหมดคือ `SELECT <label>, <value>` ที่ค่าถูก
