...
**ข้อความ duplicate ของ AddMapping / AddRule (หลังตรวจรอบ 2):** ทาง fallback ของสอง tool (เมื่อ DedupEngine ล้ม) คืน `target_condition` /
`rule_description` ของแถวที่ชน**ไม่ว่าสถานะใด** ให้โมเดลของ agent · ทางของ DedupEngine คืนแค่ id + ชนิด · แก้: ทั้งสองทางผ่าน `tools/admin/base.duplicate()` —
แถว `active` ได้เนื้อหา, แถว proposed / rejected ได้แค่ id + สถานะ + "ตัดสินที่หน้า admin / คิวข้อเสนอ" · test
`test_an_add_tools_duplicate_answer_names_a_waiting_or_rejected_row_without_its_content` ล้มบน code เดิมที่ทาง fallback · 1195 passed
