...
### อัปเดต 2026-09-19 (ค่ำ) — ebt **1.3.1** → Phase 2 exit ครบ 4/4
NT-Report `73ddc96` (build `20260919T013457Z`): สะสม 3 คอลัมน์ `agg: point_in_time`, `*_month` คง `sum`, กฎห้าม SUM/BETWEEN ข้ามงวด, รายเดือนบังคับ `*_month`, สูตรราย division/cost center (หัก `08.รายได้อื่น` + ER/MSP ในมิติเดียวกัน), ยอดนอก 2 สายงานขายไม่ใช่ยอด EBT ทางการ — ข้อมูลเท่า 1.3.0

| Exit criterion | ผล | ผ่าน? |
|---|---|---|
| 4 โดเมนตอบได้ | revenue, expense, sales, **ebt** active ทั้ง 4 | ✅ |
| eval ≥ 90% | revenue 14/14, expense 12/12, sales 12/12, **ebt 22/24 = 91.7%** (`eval_20260919_0912` บน config จริง; บนสำเนา 22/24 สองรอบ `0902`, `0906`) | ✅ |
| publish แล้ว AI เห็นเอง | build `…013457Z` + knowledge `…:1.3.1` ได้เองจากคำขอแรก ไม่ได้ลงทะเบียนใหม่ | ✅ |
| pytest | **819 passed**, 3 skipped | ✅ |

ฝั่ง AI ที่ต้องแก้ระหว่างทาง (ทั้งหมดกระทบเฉพาะ context ที่ instruction ระบุตารางอื่น = `feed_ebt`; context อื่น prompt เท่าเดิมทุก byte):
- `a458cda` filter ที่ main view ใช้ไม่ได้ห้ามถูกทิ้งเงียบ ๆ: ศูนย์ต้นทุน 2P10200 เคยได้ 8,050 M (ยอดรวมทั้งรายงาน) — Pass 2 ได้บรรทัดชี้ตาราง + ตรวจ SQL แบบ deterministic แล้ว retry
- `ff34aab` (1) guard นับเฉพาะ filter ที่ค่าอยู่ในคำถาม — Pass 1 แปลงกฎ "รายงานครอบคลุม 2 สายงาน" เป็น filter เอง ทำให้ SQL ที่ถูกถูกปฏิเสธจน retry หมด (guard ยิง 4 ครั้ง → 0); (2) Pass 1 ได้รายชื่อคอลัมน์ของตารางอื่น — เดิมบางรอบไม่ดึง filter สายงานเลย
- ถามจริง 8 ครั้ง: ใช้ filter ครบ 8/8; EBT สะสมสายงาน 1 = −214,373,476.47 (4/4 รอบ, ตรง SQL ตรวจมือ; + สายงาน 2 631,699,105.15 = 417.33 MB ตรงยอดทางการ); ศูนย์ต้นทุน 2P10200 = 313,043,027.61 (SUBTOTAL − ER ตามสูตรใหม่)
- ที่เหลือ: "ค่าใช้จ่าย…ของเดือน" ยังถูกตอบด้วยคอลัมน์ `expense` (สะสม) เป็นครั้งคราว; รายได้รายสายงานได้ยอดก่อนหัก `08.รายได้อื่น` 1 ใน 2 รอบ — ไม่มี golden รายสายงานให้วัด (ดู REMAIN-11)
