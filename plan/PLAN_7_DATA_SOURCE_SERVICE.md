...
**ผู้ใช้รายแรก:** NT-Report portal (F11 dashboard Q&A) — **go-live 2026-09-20: ฝั่ง AI เปิดแล้ว** (config.db migrate, server รัน, key จริงผูก workspace `nt-report` ออกแล้ว, `llm_provider_allowlist` = `["matcha"]`, ทุก source ok ที่ contract 2.3.1 / 1.2.1 / 1.3.1 / 1.4.1) — **ปุ่มเปิดเฉพาะ `revenue`** (2026-09-20 ดึก) มีผู้ใช้จริงแล้ว — คำถามจริง: ตัวเลขถูก 9/12 แต่ปี พ.ศ. ในข้อความผิด 7/12 (`RESULT_F11` §9); expense / sales / ebt ยังไม่เปิด (ชุดของทีม 8 / 8 / 5) · **งานต่อ: `PLAN_8_SELF_SERVICE_ONBOARDING.md`** · ผล: `plan/archive/RESULT_P7_GOLIVE.md`, `plan/archive/RESULT_F11.md`
...
- [x] E2E ผ่าน PB: **ผ่านบนของจริง 2026-09-20 ดึก** — ผู้ใช้จริงถามผ่าน portal, audit ครบทั้งสองฝั่ง
      (PB `assistant_ask` = `ok`, AI `query_audit` channel `portal`); ระหว่างทางพบ hook พังใน goja
      (`ReferenceError`) ซึ่ง node test ไม่เคยดักได้ — NT-Report แก้ + บังคับ smoke test ผ่าน PB แล้ว (`RESULT_P7_GOLIVE` §6)
