...
3. **status code ที่ hook ไม่แยก:** 403 (context ใน `ASSISTANT_CONTEXT_MAP` อยู่นอก workspace ของ key หรือถูก policy ปฏิเสธ = **config ผิด ไม่ใช่ปัญหาชั่วคราว**), 401 (key ผิด/ถูกเพิกถอน — และ**ตอนนี้ AI คืน 401 เมื่อเกินโควตารายวันด้วย**; session ฝั่ง AI อาจเปลี่ยนเป็น 429 ตามที่เจ้าของเลือก), 429 → ทั้งหมดกลายเป็น 502 ข้อความเดียวกัน และ audit เป็น `upstream_<code>`
...
5. **hook + ฝั่ง browser** (ตามที่เจ้าของเลือกในข้อ 2; ทุกการแก้มี test ตามธรรมเนียมของ repo หรือขั้นทดสอบมือที่เขียนไว้ใน ASSISTANT.md):
   ...
   - status: 403 → audit `upstream_forbidden` + ข้อความ "การตั้งค่าไม่ถูกต้อง ติดต่อผู้ดูแล" (ห้ามลองใหม่ด้วย context อื่น / ไม่มี scope); 401 → `upstream_unauthorized`; 429 (และ 401 ที่เป็นโควตา ถ้าแยกได้จาก body) → "มีผู้ใช้งานจำนวนมาก ลองใหม่ภายหลัง"; 400 คงเดิม (ไม่ตอบแบบไม่มี scope)
