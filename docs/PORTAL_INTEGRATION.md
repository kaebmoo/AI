...
- **scope ที่มีหลายงวด (2026-09-20):** prompt บอกโมเดลด้วยว่าขอบเขตคืออะไร และ **ค่ามากสุดของ key ที่เป็นตัวเลข
  คือ "งวดอ้างอิง"** — คำถามที่เอ่ยถึงเดือนหรือ "ล่าสุด" โดยไม่ระบุปีจึงหมายถึงงวดนั้น
  (ผู้เรียกที่ส่งช่วงกว้าง เช่น 13–24 เดือน ควรให้งวดของรายงานเป็น**ค่ามากสุด**ของ list เสมอ)
  จำเป็นเพราะ scope เป็น*ตัวกรอง* ไม่ใช่*จุดยึด*: ช่วงสองปีมีเดือนสิงหาคมสองครั้ง — ดู `plan/archive/RESULT_F11.md`
  คำขอที่**ไม่ส่ง** `scope` ได้ prompt เดิมทุกอย่าง
- **คำถามที่ไม่ระบุเวลาเลย = ยอดสะสมตั้งแต่ต้นปีถึงงวดอ้างอิง (2026-09-25)** — เช่น "รายได้รวม" บนรายงาน 202608 = สะสม ม.ค.–ส.ค.
  ไม่ใช่เดือน ส.ค. เดือนเดียว ("เดือนล่าสุด" / "เดือน ส.ค." ยังเป็นเดือนเดียว); ยอดรวมของทั้งปี ("ปีนี้", "ปี 2568") ก็เป็นยอดสะสม
  ณ เดือนสุดท้ายของปีนั้นในขอบเขต — คอลัมน์ยอดสะสม (point-in-time ตาม `agg` ใน contract เช่น `revenue_ytd`, `ebt`) อ่าน ณ งวดเดียว
  ไม่ SUM รายเดือน (SUM รายเดือนของ revenue พลาดยอด net ของ BG8) — pass 1 เขียน `time_range.cumulative`, code เติมเดือนสุดท้าย
  และบอก pass 2 ว่าคอลัมน์ไหนสะสม (`hybrid_flow.intent_period_note` / `build_pass2_prompt`); คำตอบบอกฐานเช่น "สะสม ม.ค.–ส.ค. 2569"
...
- portal ควรแสดงให้ผู้ใช้เห็น และเตือนเมื่อ `period` **น้อยกว่า**งวดของรายงานที่เปิดอยู่ (build ยังไปไม่ถึงงวดนั้น
  → scope จะได้ 0 แถว). `period` ที่**ใหม่กว่า**งวดของรายงานเป็นเรื่องปกติ ไม่ต้องเตือน — `scope` บังคับงวด
  ที่ชั้น SQL อยู่แล้ว คำตอบจึงเป็นงวดของรายงานเสมอ ส่วน `period` คืองวดล่าสุด**ของ bundle**
  (แก้ 2026-09-20 ตามข้อเสนอของฝั่ง NT-Report — ประโยคเดิม "เตือนเมื่อไม่ตรง" ขัดกับพฤติกรรมของ scope ในเอกสารฉบับเดียวกัน)
...
- ทุกรหัสใน `error` มาจากชุดเดียวกับ MCP facade (`app/core/outbound.py`) — เป็นค่าคงที่ ใช้ branch ได้
  ⚠️ **`error` คือ *รหัส* ไม่ใช่ข้อความอีกแล้ว** (ตั้งแต่ hardening 2026-09-20): ผู้เรียกที่เคยจับคำในข้อความ
  (เช่น หาคำว่า "publish" เพื่อรู้ว่ากำลัง publish) ต้องเปลี่ยนมาเทียบรหัส `source_unavailable` ตรง ๆ
  — ข้อความอยู่ใน `answer` ซึ่งเป็นข้อความตายตัวของรหัสนั้น
